...
const searchClient = algoliasearch(
  "GOLB1T5D3D",
  "01946b58f231b22203536c6ffa772729",
);

const search = instantsearch({
  indexName: "clients_index",
  searchClient,
  onStateChange({ uiState, setUiState }) {
    const hitsContainer = document.querySelector("#hits");
    const footerContainer = document.querySelector("#algolia-footer");

    if (!uiState["clients_index"]?.query) {
      ...
      item: (hit, { html, components }) => html`
        <div>
				  <p class="primary-text">
					  ${components.Highlight({ hit, attribute: "first_name" })}
					</p>
					<p class="secondary-text">
					  ${components.Highlight({ hit, attribute: "last_name" })}
					</p>
